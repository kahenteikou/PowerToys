...
namespace Tweetoys
{
    public class Main : IPlugin, IPluginI18n, IContextMenu, ISavable
    {
        ...
        private readonly TweetoysPluginSettings _settings;
        private readonly PluginJsonStorage<TweetoysPluginSettings> _storage;
...
#pragma warning restore CA1031 // 一般的な例外の種類はキャッチしません
        ...
        public List<Result> Query(Query query)
        {
            ...
            else
            {
                ...
                try
                {
                    IEnumerable<Result> folderPluginResults = Microsoft.Plugin.Folder.Main.GetFolderPluginResults(query);
        ...
        private List<Result> ResultsFromlHistory()
        {
            IEnumerable<Result> history = _settings.Count.OrderByDescending(o => o.Value)
                .Select(m => new Result
                {
                    ...
                    {
                        // Execute(Process.Start, PrepareProcessStartInfo(m.Key));
                        System.Windows.MessageBox.Show(m.Key, "Processkun");
                        return true;
                    },
                }).Take(5);
            return history.ToList();
